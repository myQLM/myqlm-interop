...
@brief An OQASM parser using PLY yacc tool
"""
import math # noqa
import numpy as np
import re
import ply.yacc as yacc
from .gates_ast import ASTCircuitBuilder, GateAST
...
class OqasmParser(object):
    ...
    def is_reserved(self, name):
        ...
        return (name in self.external_functions
                ...
                or self.compiler.qreg_exists(name)
                or self.compiler.creg_exists(name))
    ...
    def p_indexed_id(self, t):
        ...
        t[0].name = t[1]
        t[0].index = t[3]
        t[0].value = self.compiler.get_indexed_id(t[1], t[3])
    ...
    def p_primary(self, t):
        ...
        if isinstance(t[1], Element):  # indexed_ID
            ...
            elif self.compiler.qreg_exists(t[1].name):
                if t[1].value >= self.compiler.nbqbits:
                    raise IndexError(t[1].name+"["+t[1].index
                                      + "] is out of bounds")
                else:
                    t[0] = [t[1].value]
            elif self.compiler.creg_exists(t[1].name):
                if t[1].value >= self.compiler.nbcbits:
                    raise IndexError(t[1].name+"["+t[1].index
                                      + "] is out of bounds")
                else:
                    t[0] = [t[1].value]
    ...
    def p_if(self, t):
        ...
        c_size = self.compiler.creg_exists(t[3])
        if c_size:
            c_index = self.compiler.get_indexed_id(t[3], 0)
        else:
