...
if __name__=="__main__":
    aq = AqasmEngine(MainEngine)
    eng = AqasmPrinter(aq, engine_list=[aq])
    ...
    print("--------------------------------------------------------------")
    acirc = aq.prog.to_circ()

    from qat.mps.np import MPS
    from qat.mps.core import simulate_circuit
    #task = Task(acirc, get_qpu_server())
    #task.execute()
    #for r in task.states():
    #    print(r.state, r.amplitude)
    mps = MPS(len(aq.qb))
    simulate_circuit(mps, acirc)
