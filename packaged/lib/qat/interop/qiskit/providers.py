...
from qiskit.providers.models.backendconfiguration import BackendConfiguration, GateConfig
...
def generate_experiment_result(qlm_result, nbqbits, head):
    ...
    return ExperimentResult(shots=len(qlm_result.raw_data),
                            success=True,
                            data=data,
                            header=Obj.from_dict(head))


def generate_result(backend_name, backend_version, qobj_id, job_id, success, qlm_results, n_list, headers):
    """
    Tranform a QLM result into a qiskit result structure.
    Returns a qiskit.Result structure.
    """
    return Result(backend_name=backend_name,
                  backend_version=backend_version,
                  qobj_id=qobj_id,
                  job_id=job_id,
                  success=success,
                  results=[generate_experiment_result(result, n, head)\
...
class QLMJob(BaseJob):
    ...
    def add_results(self, qlm_result, qobj_id, n_list, headers):
        self._results = generate_result(self._backend._configuration.backend_name,
...
_QLM_PARAMS = {
        ...
        "max_shots":4096} # Max shots is 4096 (required :/)
...
class QiskitQPU(QPUHandler):
    ...
    def submit_job(self, qlm_job):
        ...
        return execute(qiskit_circuit, self).result()
